...
### 目标
- 不再引入“记忆节点”概念。
- 用户通过 `@` 直接引用当前项目内文件（含普通文件与 `.tron` 文件）。
- Agent 根据文件类型自动处理输入注入方式。
...
2. 选择器包含两个 Tab：
   - CLI 工具 Tab
   - 文件 Tab
3. 用户继续输入时执行本地实时搜索（按输入关键词过滤）。
4. 若用户停留较久（输入停顿）可触发云端插件库搜索补全。
5. 以空格作为“搜索词结束”边界，后续字符进入普通输入模式。
