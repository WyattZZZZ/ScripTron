...
- [ ] 输入框 `@` 触发下拉选择器（mention picker）。
- [ ] 下拉选择器包含双 Tab：
  - [ ] CLI 工具 Tab
  - [ ] 文件 Tab
- [ ] 搜索行为：
  - [ ] 用户输入后本地实时搜索（项目内文件 + 已安装工具）。
  - [ ] 输入停顿后触发云端插件库搜索（补全外部可安装工具）。
  - [ ] 以空格作为搜索结束边界，后续字符进入普通输入。
- [ ] 文件处理策略：
  - [ ] 非 `.tron` 文件：直接作为输入附件上传给模型。
  - [ ] `.tron` 文件：展示二级选择栏（位于原下拉栏左侧）：
    - [ ] 可执行模块（作为函数调用）
    - [ ] 文本模块（作为提示词注入）
- [ ] `.tron` 模块化规范：
  - [ ] 文件包含目录索引区（模块元信息）+ 内容区。
  ...
  - [ ] 文本模块以标题作为模块名。
- [ ] blackboard 记录：
  - [ ] 每次 `@` 选择写入 blackboard（引用类型、目标、模块名、注入方式）。
